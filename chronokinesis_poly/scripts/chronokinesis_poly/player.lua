		...
		onUpdate = function (_)
			...
			if chronokinesis_magnitude > 0 then
				if not shaderon and shaderdone then
					...
					for buh = 1,30 do
						async:newUnsavableSimulationTimer(buh*0.02, function()
							...
							shader:setFloat('strength', 0.23+buh/30*0.1)
							if buh == 30 then
								shaderdone = true
							end
						end)
				...
				if shaderdone then
					...
					shader:setFloat('strength', 0.33+clamp(math.sin(core.getRealTime()*0.645),0,1)*0.015)
				end
			else
				if shaderon then
					shaderon = false
					...
					for buh = 1,30 do
						async:newUnsavableSimulationTimer(buh*0.025, function()
							shader:setFloat('flipper', math.sin(core.getRealTime()*80))
							shader:setFloat('strength', 0.33+clamp(math.sin(core.getRealTime()*0.645),0,1)*0.015-buh/30*0.345)
						end)
					end
